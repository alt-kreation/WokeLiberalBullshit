...
//This can most likely be cut down majorly in the future once we're more familiar with Yarn Spinner
public class VoidLinePresenter : DialoguePresenterBase
{
    [field: Tooltip("What type of presenter? This is so a dialogue runner can manage multiple presenters to show dialogue in different styles.")]
    ...
    {
        _cachedDefaultFont = _dialogueTMP.font;
    }
    
    public void ApplyPersonalityParameters(PresentationPersonality personality)
    {
        if (personality.FontOverride)
        {
            _dialogueTMP.font = personality.FontOverride;
        }
    ...
    [Min(0)]
    public int typewriterEffectSpeed => 0;
    ...
    [HideInInspector]
    public Transform? AttachmentTarget = null;
    ...
    public override async YarnTask RunLineAsync(LocalizedLine line, LineCancellationToken token)
    {
        ...
        //we clear the line text immediately (this is simply styling)
        lineText.text = "";
    ...
    private void HandleProgressFeedback()
    {
        if (_progressAnimation != null)
        {
            _progressAnimation.DORestart();
        }
    }

    // can expand this out to include more types of animations and transition animations
    public bool IsAnimating()
    {
        if (!_progressAnimation) return false;
        if (_progressAnimation.tween.IsPlaying())
        {
            return true;
        }
